...
{ //Initialise queues
	...
	{
		puts("Error allocating memory for suspended ready queue");
		return -1;
	}
...
void* exitingThread(){
	...
	{//  A thread was waiting for this one to finish
		if(  (waitingThread = getThread(wThread->waitingTID, blocked)) != NULL)
		{
			waitingThread->state = PROCST_APTO;
			AppendFila2(ready, waitingThread);
		}
		else if((waitingThread = getThread(wThread->waitingTID, suspendedBlocked)) != NULL)
		{
				waitingThread->state = PROCST_APTO_SUS;
				AppendFila2(suspendedReady, waitingThread);
		}
	}
...
TCB_t* getThread(int tid, PFILA2 queue)
{
	...
	{
		FirstFila2(queue);
		while( (thread = GetAtIteratorFila2(queue)) != NULL)
		{
			// Look in queue for the waiting process
			if(thread->tid == tid)
			{	
	...
	printf("Error at getThread: Could not find thread of id: %i\n", tid);
	return NULL;
}
...
int runThread(TCB_t* thread)
{
	...
	running = thread;
	swapcontext( &(oldThread->context), &(running->context) );
	return 0;
}
...
{
	waitThread* wThread ;
	if( (wThread = (waitThread*)calloc(1, sizeof(waitThread*))) )
	{
...
int ccreate(void *(*start)(void *), void *arg, int prio)
{
	...
	if( (newThread = calloc(1, sizeof(TCB_t))) )
	{
		...
		{
			printf("Error ccreate: could not insert new Thread into ready queue\n");
			return -1;
		}
	}
	else
	{
		printf("Error ccreate: could not allocate memory for creating thread\n");
		return -1;
	}
}
...
{	
	...
	//Calls scheduler
		...
		{
			puts("Error cyield: could not dispatch next thread.");
			return -1;
		}
	}
	else
	{
		puts("Error cyield: thread could not be inserted into ready queue");
		return -1;
	}
}	
...
//Check if the system is ready
	...
	TCB_t *nextThread = calloc(1, sizeof(TCB_t*)); 
	FirstFila2(ready);
	nextThread = GetAtIteratorFila2(ready);//Gets next thread in the ready queue
	if(nextThread != NULL)//there is a thread ready
		{
			...
			if( searchWaitQueue(tid) == NULL) //No thread waiting for parameter tid
			{
				...
				{
					printf("Error cjoin: could not insert waiter Thread\n");
					return -1;
				}
			}
		else
		{
			printf("Error cjoin: Thread was already joined by another\n");
			return -1;
		}
	}
	else //No thread available
	{
		printf("Error cjoin: Could not find thread to join\n");
		return -1;
	}
}
...
int csuspend(int tid){
	if (!systemReady)
	{
		if (initialise() != 0)
		{
			printf("Error csuspend: could not init system");
			return -1;
		}
	}	
	...
	if(thread != NULL){
		...
		if(AppendFila2(suspendedReady, thread)){
			printf("Error csuspend: could not insert thread in suspendedReady queue");
			return -1;
		}else{
	...
	if(thread!=NULL){
		...
		if(AppendFila2(suspendedBlocked, thread)){
			printf("Error csuspend: could not insert thread in suspendedBlocked queue");
			return -1;
		}else{
...
int cresume(int tid){
	if (!systemReady)
	{
		if (initialise() != 0)
		{
			printf("Error cresume: could not init system");
			return -1;
		}
	}	
	TCB_t* thread = (TCB_t*) calloc(1, sizeof(TCB_t*));
	...
	if(thread != NULL){
		...
		if(AppendFila2(ready, thread)){
			printf("Error cresume: could not insert thread in ready queue");
			return -1;
		}else{
	...
	if(thread != NULL){
		...
		if(AppendFila2(blocked, thread)){
			printf("Error cresume: could not insert thread in blocked queue");
			return -1;
		}else{
